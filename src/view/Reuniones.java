...
// Territorio
   ...
   private void cargarDatosJson() {
       try (Reader reader = new java.io.InputStreamReader(
        new java.io.FileInputStream("EuskadiLatLon.json"), "UTF-8")) {
           Gson gson = new Gson();
           
           JsonElement elementoRaiz = JsonParser.parseReader(reader);
           
           if (elementoRaiz.isJsonArray()) {
               listaCentros = gson.fromJson(elementoRaiz, new TypeToken<List<Centro>>(){}.getType());
            ...
            listaCentros = gson.fromJson(arrayEncontrado, new TypeToken<List<Centro>>(){}.getType());
            } else {
           ...
           if (listaCentros != null) {
            ...
            List<String> territorios = listaCentros.stream()
            .map(Centro::getTerritorio)
            .filter(t -> t != null) // Evitar nulos
                       .distinct()
                       .sorted()
   ...
   private void configurarEventos() {
       ...
       cbxMunicipio.addActionListener(e -> {
           ...
           if (munSel == null || munSel.equals("Selecciona...")) return;
           ...
           List<String> centros = listaCentros.stream()
               .filter(c -> {
                   if (c.getMunicipio() == null) return false;
